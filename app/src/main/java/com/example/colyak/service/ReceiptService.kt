            ...
            return withContext(Dispatchers.IO) {
                try {
                    ...
                    if (response.isSuccessful) {
                        ...
                        Log.e("ReceiptService", "getAll request failed with code: $errorCode, message: $errorMessage")
                        null
                    }
            ...
            return withContext(Dispatchers.IO) {
                try {
                    ...
                    if (response.isSuccessful) {
                        ...
                        val errorCode = response.code()
                        val errorMessage = response.errorBody()?.string()
                        Log.e("ReceiptService", "getFavorite5Receipts request failed with code: $errorCode, message: $errorMessage")
                        null
                    }
